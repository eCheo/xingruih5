<template>
  <div>
    <van-nav-bar title="客户详情" left-text="返回" left-arrow @click-left="$router.push('/mycustomer')"></van-nav-bar>
    <van-form>
      <van-field
      ...
      <van-field readonly name="需求区域" label="需求区域" :value="areaText" placeholder="需求区域" />
      <van-field
      ...
      <div style="padding:0 16px;margin: 10px 0;height: 41px;">
        <span
          style="color:#646566;padding-top: 10px;display: inline-block;"
        >电话号码：{{cusInfo.isViewPhone ? cusInfo.hidPhone : cusInfo.phone}}</span>
        <mu-button
          style="margin-left:15px;float:right;"
          v-if="cusInfo.isViewPhone"
          color="success"
          @click="viewPhone"
        >查看电话号码</mu-button>
      </div>
    ...
    <mu-list textline="two-line" style="background-color: #fff;">
      <mu-list-item
        ...
        :ripple="false"
        button
        v-for="(item, index) in sharedData"
        :key="index"
    ...
    findById() {
      ...
      findById(params).then(res => {
        if (res.status === 200 && res.data.code === "200") {
          ...
          this.areaText =
            (this.cusInfo.areaName || "--") +
            " " +
            (this.cusInfo.streetName || "--");
          this.loading = false;
        } else {
          this.$toast.fail(res.data.message);
          this.loading = false;
